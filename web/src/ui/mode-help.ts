...
function ensureEl(): HTMLDivElement {
  const existing = document.getElementById('modeHelp')
  let el: HTMLDivElement | null = existing instanceof HTMLDivElement ? existing : null
  if(!el){
...
function contentFor(mode: string | null | undefined): string {
...
export function showModeHelp(mode: string | null | undefined): void {
  const el = ensureEl()
  const content = el.querySelector<HTMLDivElement>('.content')
  if(content) content.innerHTML = contentFor(mode)
