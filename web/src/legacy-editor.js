...
import { $$, vn, snap as snapToGrid, genIdWithTime as genId, incrementName, isCanal, download } from './utils.js'
...
function snap(v){ return snapEnabled ? snapToGrid(v, gridStep) : v }
...
// Exports
document.getElementById('exportBtn').onclick=()=>{ const data = toJSON({nodes,edges}); download(JSON.stringify(data,null,2),'reseau.json') }
document.getElementById('exportCompactBtn').onclick=()=>{ const out = toCompact({nodes,edges}); download(JSON.stringify(out),'reseau-compact.json') }
document.getElementById('exportNodeEdgeBtn').onclick = ()=> { const compiled = toNodeEdge({nodes,edges}); const data = JSON.stringify(compiled, null, 2); download(data, 'reseau-node-edge.json'); status('Export nœud/arête généré.') }

// compileNodeEdgeGraph moved to exports.js → toNodeEdge()
