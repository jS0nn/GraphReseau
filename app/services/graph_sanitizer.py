"""Utilities to normalise graph payloads before persisting.

This module centralises the sanitisation previously embedded in
``app/datasources.py`` so that all backends share the same behaviour.
"""
...
def _gen_edge_id() -> str:
    """Generate a short stable edge identifier."""
...
def sanitize_graph_for_write(graph: Graph | None) -> Graph:
    """Ensure edges are valid before persisting a graph.

    - Keep the first occurrence of a given id/from/to combination.
    - If an id is reused for different endpoints, assign a fresh one.
    - Assign an id when missing.
    - Drop edges missing endpoints.
    """
    if graph is None:
